# -*- coding: utf-8 -*-
"""
Created on Sun Nov  3 12:15:50 2019
...
mdf = pd.DataFrame(metin_vektoru , columns = ["Messages"])
    
d_mdf = mdf.copy()

d_mdf["Messages"].apply(lambda x: " ".join(x.lower() for x in x.split()))

## NOKTALAMA İŞARETLERİNİN SİLİNMESİ

d_mdf = d_mdf["Messages"].str.replace("[^\w\s]","")

###Sayıların silinmesi
d_mdf = d_mdf.str.replace("\d","")

## STOPWORDS
d_mdf = pd.DataFrame(d_mdf , columns = ["Messages"])
import nltk

nltk.download("stopwords")
from nltk.corpus import stopwords
sw = stopwords.words("english")
d_mdf["Messages"].apply(lambda x: " ".join(x for x in x.split() if x not in sw ))

## Deleting less words

import pandas as pd

sil = pd.Series(" ".join(d_mdf["Messages"]).split()).value_counts()[-11000:]
d_mdf["Messages"].apply(lambda x: " ".join(x for x in x.split() if x not in sil ))
...
d_mdf["Messages"].apply(lambda x : TextBlob(x).words)

data2 = d_mdf["Messages"].apply(lambda x : TextBlob(x).words)
d_mdf = pd.DataFrame(d_mdf , columns = ["Messages"])

ccc = data2.copy()
ccc = pd.DataFrame(ccc , columns = ["Messages"])
...
d_mdf["Messages"].apply(lambda x: " ".join([Word(word).lemmatize() for word in x.split() ] ))
